...
void *myroute(void *arg)
{
	...
	do {
		...
		case RequestCodec::NewOrUpdate:
			ret = pInfo->s->secKeyAgree(pRequestMsg, &sendData, sendLen); //2.free
			break;

			//密钥校验
		case RequestCodec::Check:
			ret = pInfo->s->secKeyCheck(pRequestMsg, &sendData, sendLen); //2.free
			break;

			//密钥注销
		case RequestCodec::Revoke:
			ret = pInfo->s->secKeyRevoke(pRequestMsg, &sendData, sendLen); //2.free
			break;
...
void ServerOperation::startWork()
{
    ...
    //连接Oracle数据库
    mOp->connectDB(mInfo->dbUser, mInfo->dbPasswd, "192.168.14.45:1521/orcl");
